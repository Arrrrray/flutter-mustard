...
class SPUI {
  // 我们自己设定的虚拟屏幕宽度750
  static double customScreenWidth = 750.0;
  ...
  /// 中间改成了使用[window]中的属性，获得屏幕大小进行初始化。
  /// 2.3.0版本后面又改回了使用[MediaQuery]，好像是在某某情况下window获得的屏幕大小不准确
  /// 目前我们锁死使用2.2.0版本。
  static void init() {
    // 自定义屏幕适配器比例（物理宽度/设定宽度）
    double customScreenRatio = physicalSize.width / customScreenWidth;

    ScreenUtil.init(
      width: customScreenWidth,
      height: physicalSize.height / customScreenRatio,
    );
  }
  ...
  static double get bottomBarHeight =>
      ScreenUtil.bottomBarHeight / ScreenUtil.pixelRatio;
