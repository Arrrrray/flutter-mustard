...
import '../../utils/sparrow_utils.dart';
...
class SPGridContainerExpandedWidth extends StatelessWidget {
  final int crossAxisCount;
  final double itemSpacing;
  final double itemHeight;
  final double rowSpacing;
  final List<Widget> children;
  final Color backgroundColor;
  const SPGridContainerExpandedWidth(
      {Key key,
      @required this.crossAxisCount,
      @required this.children,
      @required this.itemHeight, // 如果不设置高度就等于宽度
      double itemSpacing,
      double rowSpacing,
      Color backgroundColor})
      : this.itemSpacing = itemSpacing ?? 0.0,
...
//
  ...
  Widget build(BuildContext context) {
    ...
    for (var i = 0; i < count; i += (crossAxisCount)) {
      ...
      // 如果最后一行不足个数，用空白格补全
      while (items.length < crossAxisCount) {
        items.add(Expanded(child: Container(height: gridHeight)));
      }
      rows.add(
        Container(
          color: backgroundColor,
